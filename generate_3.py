...
# image in pdf is cropped due to heavy size + text is also blurred now
def generate_receipt_from_image(row, template_path, output_path):
    try:
        template = Image.open(template_path)
        draw = ImageDraw.Draw(template)

        # Coordinates for text (scaled for high-resolution template)
        scale_factor = 2550 / 6480  # Scale to fit 8.5 inches at 300 DPI
        date_pos = (int(50 * scale_factor), int(30 * scale_factor))
        name_pos = (int(50 * scale_factor), int(70 * scale_factor))
        id_pos = (int(50 * scale_factor), int(110 * scale_factor))
        phone_pos = (int(50 * scale_factor), int(150 * scale_factor))
        batch_pos = (int(50 * scale_factor), int(190 * scale_factor))
        amount_pos = (int(50 * scale_factor), int(230 * scale_factor))

        # Adjust font size for scaled image
        scaled_font_size = int(font_size * scale_factor)
        scaled_font = ImageFont.truetype(font_path, scaled_font_size)

        # Draw text
        draw.text(date_pos, f"Date: {datetime.now().strftime('%Y-%m-%d')}", fill="black", font=scaled_font)
        draw.text(name_pos, f"Name: {row['Name']}", fill="black", font=scaled_font)
        draw.text(id_pos, f"ID: {row['ID']}", fill="black", font=scaled_font)
        draw.text(phone_pos, f"Phone: {row['Phone']}", fill="black", font=scaled_font)
        draw.text(batch_pos, f"Batch: {row['Batch']}", fill="black", font=scaled_font)
        draw.text(amount_pos, f"Payment Amount: ${row['PaymentAmount']}", fill="black", font=scaled_font)

        # Resize image to fit letter-sized PDF (8.5x11 inches at 300 DPI)
        target_width = 2550  # 8.5 inches at 300 DPI
        target_height = int(target_width * (11520 / 6480))  # Maintain 9:16 aspect ratio
        ...
        resized_template.save(temp_image_path, quality=95)

        # Create PDF
        receipt_path = os.path.join(output_path, f"{row['ID']}_Receipt_Image.pdf")
        c = canvas.Canvas(receipt_path, pagesize=letter)
        img = ImageReader(temp_image_path)
        c.drawImage(img, 0, letter[1] - target_height, width=target_width, height=target_height)
        c.save()
