...
def generate_receipt_from_image(row, template_path, output_path):
    try:
        ...
        if target_height > 3300:  # Cap height at 11 inches (3300 pixels at 300 DPI)
            target_height = 3300
            target_width = int(target_height * (6480 / 11520))  # Adjust width to maintain aspect ratio
        ...
        # Coordinates for text (adjust based on resized template)
        date_pos = (50, 30)
        name_pos = (50, 70)
        id_pos = (50, 110)
        phone_pos = (50, 150)
        ...
        draw.text(date_pos, f"Date: {datetime.now().strftime('%Y-%m-%d')}", fill="black", font=font)
        draw.text(name_pos, f"Name: {row['Name']}", fill="black", font=font)
        draw.text(id_pos, f"ID: {row['ID']}", fill="black", font=font)
        draw.text(phone_pos, f"Phone: {row['Phone']}", fill="black", font=font)
        draw.text(batch_pos, f"Batch: {row['Batch']}", fill="black", font=font)
        draw.text(amount_pos, f"Payment Amount: ${row['PaymentAmount']}", fill="black", font=font)
        ...
        y_offset = letter[1] - target_height  # Align top of image to top of page
        if y_offset < 0:  # If image is too tall, adjust to fit
            y_offset = 0
