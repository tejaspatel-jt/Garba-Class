...
font2 = os.path.join(base_path, "WDXLLubrifontJPN-Regular.ttf")
font3 = os.path.join(base_path, "EduNSWACTHandPre-VariableFont_wght.ttf")
...
def generate_id_card(row, template_path, output_path):
    ...
    # Draw text - This is for Demo Purpose only ⚠️
    draw.text(name_pos, f"{row['Name']}", fill="black", font=ImageFont.truetype(font2, 33))
    draw.text(id_pos, f"{row['ID']} - {row['Batch']}", fill="black", font=ImageFont.truetype(font_path, 30))
    draw.text(phone_pos, f"{row['Phone']}", fill="black", font=ImageFont.truetype(font2, 30))
    draw.text(address_pos, f"{row['Address']}", fill="black", font=ImageFont.truetype(font2, 30))
